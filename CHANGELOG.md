...
**전원이 꺼진 상태에서 켜면 맞춰둔 온도가 최저로 돌아가던 문제를 고쳤습니다** (#19).

### 무엇이 문제였나

v0.17.2 부터, 온도형 매트의 **전원이 꺼진 상태**에서 난방을 켜면 맞춰둔 온도(예: 33℃)가 무시되고 **최저 온도 28℃** 로 켜졌습니다.

기기는 전원이 꺼지면 모든 구역에 「난방 안 함」 표시를 붙입니다 — 맞춰둔 온도와 무관하게요. 저희가 그 표시를 「이 구역은 꺼진 상태로 설정됐다」로 잘못 읽고, 켤 때 온도를 최저로 되돌렸습니다. 제보해 주신 기록 덕분에 바로 찾았습니다.

이제 **전원이 켜져 있을 때만** 그 표시를 믿습니다. 전원이 꺼져 있으면 맞춰둔 온도를 그대로 유지합니다.

### 「마지막 남은 쪽을 끄면 전원도 꺼지게 해달라」는 이번에 넣지 않았습니다

검토를 위해 나비엔 앱을 다시 뜯어봤습니다. **앱도 이 동작을 하지 않습니다** — 같은 상황에서 막고 안내만 띄웁니다. 이유가 있었습니다.

- 앱에서 전원을 켜고 끄는 것은 **오직 전원 버튼 하나**입니다. 온도 조작이 전원을 건드리는 경우가 한 곳도 없습니다
- 보내주신 기록에서 확인된 기기 동작: **전원을 켜면 양쪽이 전부 켜집니다.** 한쪽만 쓰다가 전원을 껐다 켜면 그 구성이 사라집니다

자세한 것은 이슈 #19 의 답변에 적었습니다. 전부 끄실 때는 전원 스위치를 쓰시면 되고, 자동화로 원하는 동작을 만드는 방법도 안내해 두었습니다.
...
| --- | --- |
| 카본(단계형) 매트 | 영향 없습니다 |
| 온도형 — 전원 켜진 상태의 조작 | 그대로 |
| 온도형 — 전원 꺼진 상태에서 켜기 | **맞춰둔 온도가 살아납니다** (v0.17.2~v0.18.0 은 28℃로 돌아갔음) |
| 환기청정 · 전원 스위치 · 필터 | 무관 |
