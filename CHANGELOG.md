...
- **v0.4.x 에서 올라오는 절차** — 삭제 후 재등록이 필요하다
- **HACS 가 구 폴더를 안 지운다** — 실측 확인했다. `custom_components/navien_smart/`
  를 직접 지워야 한다. 남으면 통합 추가 목록에 같은 이름이 둘 뜨고, 구 폴더도
  `config_flow` 가 있어 잘못 고르면 옛 버전이 올라온다
