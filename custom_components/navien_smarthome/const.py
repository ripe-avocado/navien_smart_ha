...
# 모든 구형에 바이패스를 붙였다.
#
# 제보(#13, `NRT-20D`)로 없는 기기가 있다는 것이 드러났다. 앱에 안 보이는데
# 우리는 보여줬고, 눌렀더니 실외기가 서고 전력이 3W 로 떨어졌다(환기·미풍 51W).
#
# **처음에는 「`DS` 붙은 것만 보여준다」로 만들었다가 되돌렸다.** 사용자가 자료를
# 더 찾았는데 `NRT-30` 과 `NTR-10PW` 도 바이패스가 있었다. 그 방식이면 그 계열이
# 되던 기능을 잃는다.
#
# | 모델 | 바이패스 | 근거 |
# | --- | --- | --- |
# | `NRT-20D` | **없음** | 제보 #13 — 앱에 없고 눌러도 안 된다 |
# | `NRT-21D` | **없음** | 제품 자료 — `S` 가 없는 기본형 |
# | `NRT-20DS` · `NRT-20DSW` | 있음 | 앱 화면 · 17 명령 적용 확인 |
# | `NRT-30` | 있음 | 제품 자료 |
# | `NTR-10PW` | 있음 | 제품 자료 — 계열 자체가 다르다 |
#
# `NRT-xxD` 가 기본형이고 `DS` 가 그 위다. **다만 그 규칙으로 거르지 않는다** —
# `NTR-10PW` 는 `S` 가 없는데 바이패스가 있다. 계열이 다르면 규칙도 다르다.
#
# 그래서 **확인된 이름만 뺀다.** 넓게 막으면 되던 기기를 깨뜨리고, 좁게 막으면
# 제보 한 건으로 넓힐 수 있다. 되돌리기 쉬운 쪽을 고른다.
#
# 비교는 대소문자와 하이픈을 무시한다 — 서버가 `NRT20D` 로 줄 수도 있다.
LEGACY_NO_BYPASS_MODELS: Final = frozenset({"NRT20D", "NRT21D"})
