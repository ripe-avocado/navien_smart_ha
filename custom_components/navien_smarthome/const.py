...
# 구세대 DID 는 **모드 목록 자체를 주지 않는다** — 변형이 붙은 모드만 나온다.
# 앱 제어화면은 6개를 보여주므로 실기기(NRT-20DSW)에서 하나씩 걸어 확인한
# 코드만 둔다. 숙면은 DID 의 `(4, 4)` 로 이미 들어온다.
#   4 환기 · 8 청정 · 12 자동운전 · 6 요리 · 17 바이패스
# 모델이 다르면 지원 범위도 다를 수 있다. 기기가 받지 않는 코드는 상태가 따라
# 오지 않으므로 표시가 되돌아가고, 잘못된 동작을 만들지는 않는다.
LEGACY_VERIFIED_MODES: Final = (4, 8, 12, 6, 17)
