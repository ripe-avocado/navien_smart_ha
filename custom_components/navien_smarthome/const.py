...
#
# (모드, 옵션, 기본 풍량) — 풍량은 그 조합의 기본값이고 고를 수 있는 목록이 아니다.
LEGACY_MODE_DID: Final = (
    ...
    (4, 2, 3),    # 환기 터보
    (4, 3, 4),    # 환기 절전
    (4, 4, 4),    # 숙면 — 앱은 (4,4) 를 별도 모드로 보여준다
    (8, 1, 1),    # 청정
    (8, 2, 3),    # 청정 터보
    (8, 3, 4),    # 청정 절전
    (17, 1, 4),   # 바이패스 — 내장 파일에는 없고 실기기 앱 화면에 있다
)

# 구세대 DID 에는 `supportedAirVolumes` 가 없다. 기본 조합에서 고를 수 있는
# 풍량은 앱 표를 그대로 쓴다 — 기여자가 실기기에서 미풍·약풍·강풍·자동이 모두
# 동작하는 것을 확인했다.
