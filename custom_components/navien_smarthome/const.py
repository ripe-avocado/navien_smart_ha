...
#
# `unit` 은 **앱에 실제로 있는 단위 문자열만** 쓴다. dex 전수 검색 결과 앱이 쓰는
# 공기질 단위는 `"ppm"` 과 `"㎍/㎥"` 둘뿐이다. 온도·습도는 앱 밖의 표준 기호다.
#
# 그래서 `tvoc` `radon` `total` 은 단위를 비운다. 앱도, 기기 화면도 이 셋에
# 단위를 쓰지 않는다 (라돈은 「라돈 관리 단계」와 등급으로만 보여준다).
#
# **라돈에 `Bq/㎥` 를 붙였다가 걷어냈다.** 국제 표준이라 그럴듯하고 먼저 나온
# 통합도 그렇게 하지만, 앱에 근거가 없다. 라돈은 발암물질이고 국내 기준이
# 148 Bq/㎥ 다 — 단위를 잘못 붙이면 사용자가 건강 판단을 틀리게 한다.
# **모르면 숫자만 낸다.** 등급은 `grade` 속성에 있다.
#
# `device_class` 는 HA 가 아이콘·히스토리 그래프·단위 변환에 쓴다.
